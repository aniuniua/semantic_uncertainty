"""Sample answers from LLMs on QA task, with optional single-answer mode that
logs per-token probabilities (for margin-style analysis)."""
...
def main(args):
    ...
    # In single-answer mode we treat this as a diagnostic run:
    # disable expensive uncertainty computation by default to keep things simple.
    if args.single_answer_mode:
        if args.compute_uncertainties:
            logging.info(
                'single_answer_mode is True: overriding `compute_uncertainties=False` '
                'to avoid downstream scripts that expect multiple generations.'
            )
        if args.compute_p_true:
            logging.info(
                'single_answer_mode is True: overriding `compute_p_true=False` '
                'because p_true relies on multiple generations.'
            )
    ...
    # Initialize prompt for p_true baseline (skipped in single_answer_mode above).
    if args.compute_p_true:
        logging.info(80 * '#')
    ...
    logging.info(80 * '=')
    logging.info('Generating answers (margin script): ')
    logging.info(80 * '=')
    for dataset_split in ['train', 'validation']:
        ...
        for index in tqdm(indices):
            ...
            # Decide how many generations to sample.
            if args.single_answer_mode:
                num_generations = 1
            elif dataset_split == 'train' and args.get_training_set_generations_most_likely_only:
                num_generations = 1
            else:
            ...
            for i in range(num_generations):
                ...
                if i == 0:
                    ...
                    logging.info('prediction: '.ljust(15) + predicted_answer)
...
if __name__ == '__main__':
    ...
    logging.info('STARTING `generate_answers_margin`!')
    main(args)
    logging.info('FINISHED `generate_answers_margin`!')
